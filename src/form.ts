...
const roundRobinSeeds: SeedOrdering[] = [
  'groups.effort_balanced',
  'groups.seed_optimized',
  'groups.bracket_optimized'
]

const eliminationOrderings: SeedOrdering[] = [
  'natural',
  'reverse',
  'half_shift',
  'reverse_half_shift',
 ...
 */
export function stageFormCreator(
    ...
    switch ((<HTMLInputElement>stageSelector).value) {
      ...
      default:
        throw new DOMException(
          'stage ' + (<HTMLInputElement>stageSelector).value + ' seems to be not implemented yet.'
        )
    }
 ...
 */
export function updateFormCreator(
  configuration: FormConfiguration,
...
function createMaskFields(
  ...
  switch (stage) {
    case 'round_robin':
      ...
      createSelect(
        parent,
        config.html_round_robin_mode_id,
        t('form-creator.round-robin-mode-label'),
        roundRobinMode
      )
    ...
    case 'single_elimination':
      ...
      createInput(
        parent,
        'checkbox',
        config.html_consolation_final_checkbox_id,
        t('form-creator.consolation-final-label')
      )
    ...
    case 'double_elimination':
      ...
      createInput(
        parent,
        'checkbox',
        config.html_skip_first_round_checkbox_id,
        t('form-creator.skip-first-round-label')
      )

      // Grand final types
      createSelect(
        parent,
        config.html_grand_final_type_id,
        t('form-creator.grand-final-type-label'),
        grandFinalTypes
      )
  ...
  switch (stage) {
    case 'round_robin':
      ...
      const roundRobinSettings: StageSettings = {
        seedOrdering: [
          <SeedOrdering>(
            (<HTMLInputElement>document.getElementById(config.html_seed_order_id)).value
          ) ?? 'groups.effort_balanced'
        ],
        roundRobinMode:
          <RoundRobinMode>(
            (<HTMLSelectElement>document.getElementById(config.html_round_robin_mode_id)).value
          ) ?? 'simple',
        groupCount: parseInt(
          (<HTMLInputElement>document.getElementById(config.html_group_id)).value ?? '0'
        )
      }
      ...
      teamNamesValue = (<HTMLTextAreaElement>(
        document.getElementById(config.html_team_names_input_id)
      ))?.value

      if (teamNamesValue) payload.seeding = teamNamesValue.split(',')
      else {
        const teamCount = parseInt(
          (<HTMLTextAreaElement>document.getElementById(config.html_team_count_input_id)).value
        )
    ...
    case 'single_elimination':
      ...
      const singleEliminationSettings: StageSettings = {
        ...
        consolationFinal: (<HTMLInputElement>(
          document.getElementById(config.html_consolation_final_checkbox_id)
        )).checked
      }
    ...
    case 'double_elimination':
      ...
      const rawSeedOrder = (<HTMLTextAreaElement>(
        document.getElementById(config.html_double_elimination_seed_textarea_id)
      )).value.split(',')
      ...
      const doubleEliminationSettings: StageSettings = {
        ...
        skipFirstRound: (<HTMLInputElement>(
          document.getElementById(config.html_skip_first_round_checkbox_id)
        )).checked,
        grandFinal: <GrandFinalType>(
          (<HTMLSelectElement>document.getElementById(config.html_grand_final_type_id)).value
        )
      }
...
function validateDoubleElimination(config: FormConfiguration): void {
  ...
  const grandFinalType = (<HTMLSelectElement>(
    document.getElementById(config.html_grand_final_type_id)
  )).value as GrandFinalType
  ...
  const orderings = (<HTMLTextAreaElement>(
    document.getElementById(config.html_double_elimination_seed_textarea_id)
  )).value.split(',')
  ...
  orderings.forEach((value) => {
    ...
    if (!eliminationOrderings.includes(ordering))
      throw new DOMException(
        'elimination seed_ordering wrong found: ' +
          ordering +
          'must be one of: ' +
          eliminationOrderings.toString()
      )
  })
}
...
function validateSingleElimination(config: FormConfiguration): void {
  ...
  const ordering = (<HTMLSelectElement>document.getElementById(config.html_seed_order_id))
    .value as SeedOrdering
...
function validateRoundRobin(config: FormConfiguration): void {
  ...
  const groupAmount = parseInt(
    (<HTMLInputElement>document.getElementById(config.html_group_id)).value
  )
  ...
  const roundRobinMode = (<HTMLSelectElement>(
    document.getElementById(config.html_round_robin_mode_id)
  )).value
...
function baseValidation(config: FormConfiguration): void {
  ...
  const teamNamesValue = (<HTMLInputElement>(
    document.getElementById(config.html_team_names_input_id)
  ))?.value
  const teams = teamNamesValue ? teamNamesValue.split(',') : []
  const teamCount =
    teams.length ||
    parseInt((<HTMLInputElement>document.getElementById(config.html_team_count_input_id)).value)

  if (teamCount < 2) throw new DOMException('Invalid team amount provided.')

  const stageType = (<HTMLInputElement>document.getElementById(config.html_stage_type_selector_id))
    .value as StageType
